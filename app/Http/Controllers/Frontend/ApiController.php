...
class ApiController extends Controller
{
   ...
   public function checkout(Request $request)
    ...
    use RegistersUsers;
    public function currentuser($phone_number)
    {
        //$wishlist->user_id = Auth::user()->id;
        $user = Auth::user()->where(Auth::user()->phone_number,$phone_number);//::where('phone_number',Auth::user()->$phone_number);
        //$user = User::where('phone_number', Auth::check()->find($phone_number));
        return response()->json($user,200);
    }
