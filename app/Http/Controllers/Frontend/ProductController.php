...
class ProductController extends Controller
{
    ...
    public function apiIndex() {
        ...
        //$att = Product::orderBy('id','desc')->pluck('attribute_options');
        
        $arr = array();
        for($i = 0; $i < count($products); $i++){
            $images = ProductImage::where('product_id', $products[$i]->id)->pluck('image');
            $att = $products[$i]->attribute_options;
            $subAtt = substr($att, 32);
            $test = strrpos($subAtt, "৳");
            
            $sub = substr($subAtt, 0, $test+3);
            // return $sub;
            // $at = unserialize($att);
            // $test = implode(",", $at);
            ...
            //$ttt = "dfdf";
            //$at = json_encode($att);
            //list($keys, $values) = array_divide($at);
           $products[$i]['att'] = $sub;
            //$products[$i]['att'] = $at;//serialize($at);
    ...
    public function index() {
     ...
     public function showBySubcategory($subcategoryId) {
         $products = Product::where('category_id',$subcategoryId)->orderBy('id','desc')->get();
         if (!is_null($products)) {
            return view('frontend.pages.product.product_subcategory_all',compact('products'));
        }else{
            return view('frontend.pages.product.noproduct');
